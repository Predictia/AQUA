...
class sshVariability():
    ...
    def run(self):
        self.compute_std()
        if self.ref_std is not None:
            self.plot_std(self.ref_std, self.name_ref, self.exp_ref, self.startdate_ref, self.enddate_ref, contours=21)
            self.region_selection(self.ref_std, self.name_ref,self.exp_ref)
        if seld.model_std is not None:
            self.plot_std(self.model_std, self.name_model, self.exp_model, self.startdate_model, self.enddate_model, contours=21)
        ...
        self.plot_std_diff(self, contours=21)
