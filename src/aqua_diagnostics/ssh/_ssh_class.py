...
class sshVariability():
    ...
    def save_netcdf(self):
        ...
        if self.ref_std is not None:
            output_file = os.path.join(file_type_folder, f"{self.ref_name}_{self.ref_exp}_{self.startdate_ref}_to_{self.enddate_ref}_std.nc")
            self.ref_std.to_netcdf(output_file)
        if self.model_std is not None:
            output_file = os.path.join(file_type_folder, f"{self.model_name}_{self.model_exp}_{self.startdate_model}_to_{self.enddate_model}_std.nc")
    ...
    def plot_std_diff(self, contours=21):
        ...
        if self.ref_std is not None and self.model_std is not None:
            #to remove large fill values
            #ref_std = xr.where(self.ref_std < 100, self.ref_std, np.nan) # <- not sure about this
            diff_data = self.ref_std - self.model_std
        else:
            Print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ERROR !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
