...
def retrieve_merge_ensemble_data(variable: str = None, region:str = None, data_path_list: list[str] = None, models_catalog_list: list[str] = None, exps_catalog_list: list[str] = None, sources_catalog_list: list[str] = None, startdate: str = None, enddate: str = None, log_level: str = "WARNING"):
    """
    ...
    if data_path_list is not None:
        ens_dataset = xr.concat([xr.open_dataset(f).expand_dims(ensemble=[i]) for i, f in enumerate(data_path_list)],dim='ensemble')
        if variable is not None:
            ens_dataset = ens_dataset.sel(variable=variable)
