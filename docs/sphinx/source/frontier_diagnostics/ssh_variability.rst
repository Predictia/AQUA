...
The **sshVariability** diagnostic calculates the sea surface height (SSH) standard deviation for models (e.g. FESOM, ICON, NEMO)
...
* **BaseMixin**: this class is called inside the **sshVariabilityCompute** class.
  This class basically retrieves the data and provides the functionality to save the output as netcdf file.
...
The diagnostic is designed to work with both the data from the Low Resolution Archive (LRA) and the original high resolution Healpix data. The LRA is generated by the Data reduction OPerator (DROP) of the AQUA project, which provides monthly data at a 1x1 
degree resolution. 
