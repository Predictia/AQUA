...
def main():
    ...
    save_standard_deviation_to_file(config['output_directory'], "AVISO", aviso_ssh_std)
    ...
    for model_name in config['models']:
        ...
        reader = Reader(model=model_name['name'], exp=model_name['experiment'], source=model_name['source'], regrid=model_name['regrid'])
        model_data = reader.retrieve(fix=False)
        
        ssh_data = model_data['ssh']
        
        print("Getting SSH data complete for {}, now computing standard deviation on the default timestamp".format(model_name))
        # computing std
    ...
    visualize_subplots(ssh_data_list, fig, axes)

    print("Saving plots as JPEG output file")
    save_subplots_as_jpeg(config, "subplots_output.jpeg", fig)
