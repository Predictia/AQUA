...
def select_timeseries_type(data, region=None, type = None, latS: float=None, latN: float=None, lonW: float=None,
                            lonE: float=None):
    type = type.lower()
    data  = weighted_area_mean(data, region, latS, latN, lonW, lonE)
    if type is None:
        ...
        anomaly = xr.Dataset()
        for var in list(data.data_vars.keys()):
            anomaly[var] = data[var] - data[var].isel(time=0)
        data = anomaly
    elif type == 'StdAnomaly':
        std_anomaly = xr.Dataset()
        for var in list(data.data_vars.keys()):
            anom = data[var] - data[var].isel(time=0)
            anom.attrs['units'] = 'Standardised Units'
            # Calculate the standard anomaly by dividing the anomaly by its standard deviation along the time dimension
            std_anomaly[var] = anom / anom.std(dim="time")
        data = std_anomaly
...
def hovmoller_plot(data, region, type , latS: float=None, latN: float=None, lonW: float=None,
    ...
    data = select_timeseries_type(data, region, type, latS, latN, lonE, lonW)
    ...
    cs1=axs[0].contourf(data.time,lev_log_scale, data.ocpt.transpose(),
                        levels=np.linspace(np.min(data.ocpt),np.max(data.ocpt),10),cmap='viridis',extend='both')
    cbar_ax = fig.add_axes([0.15, 0.1, 0.35, 0.05])
    cbar = fig.colorbar(cs1, cax=cbar_ax, orientation='horizontal')
    cbar.set_ticks((np.linspace(np.min(data.ocpt),np.max(data.ocpt),10)).tolist())  # Set the tick positions
    
    
    cs2=axs[1].contourf(data.time,lev_log_scale, data.so.transpose() ,
                        levels=np.linspace(np.min(data.so),np.max(data.so),14),cmap='viridis',extend='both')
    cbar_ax = fig.add_axes([0.55, 0.1, 0.3, 0.05])
    cbar = fig.colorbar(cs2, cax=cbar_ax, orientation='horizontal')
    cbar.set_ticks((np.linspace(np.min(data.so),np.max(data.so),14)).tolist())  # Set the tick positions
...
def multilevel_t_s_trend_plot(data, region, customise_level=False, levels=None, outputfig="./figs"):
    """
