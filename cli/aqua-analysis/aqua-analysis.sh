...
args="--model $model --exp $exp --source $source"
...
if [ "$atmglobalmean" = true ] ; then
  ...
  python $aqua/diagnostics/atmglobalmean/cli/cli_atmglobalmean.py $args --outputdir $outputdir/atmglobalmean
fi
...
if [ "$global_time_series" = true ] ; then
  ...
  python $aqua/diagnostics/global_time_series/cli/cli_global_time_series.py $args --outputdir $outputdir/global_time_series
fi

if [ "$ocean3d" = true ] ; then
  echo "Running ocean3d"
  python $aqua/diagnostics/ocean3d/cli/cli_ocean3d.py $args --outputdir $outputdir/ocean3d
fi

if [ "$radiation" = true ] ; then
  echo "Running radiation"
  python $aqua/diagnostics/radiation/cli/cli_radiation.py $args --outputdir $outputdir/radiation
fi

if [ "$seaice" = true ] ; then
  echo "Running seaice"
  python $aqua/diagnostics/seaice/cli/cli_seaice.py $args --outputdir $outputdir/seaice
fi

if [ "$teleconnection" = true ] ; then
  echo "Running teleconnection"
  python $aqua/diagnostics/teleconnection/cli/cli_teleconnection.py $args --outputdir $outputdir/teleconnection
fi
