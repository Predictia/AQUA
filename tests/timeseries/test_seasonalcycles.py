...
class TestSeasonalCycles:
    ...
    def test_no_region(self, tmp_path):
        ...
        filename = f'{self.diagnostic_name}.seasonalcycles.{self.var}.monthly.{self.catalog}.{self.model}.{self.exp}.nc'
        ...
        filename = f'{self.diagnostic_name}.seasonalcycles.{self.var}.monthly.std.{self.catalog}.{self.model}.{self.exp}.nc'
        file = os.path.join(tmp_path, 'netcdf', filename)
        assert os.path.exists(file)
        ...
        filename = f'{self.diagnostic_name}.seasonalcycle.{self.catalog}.{self.model}.{self.exp}.{self.var}.png'
