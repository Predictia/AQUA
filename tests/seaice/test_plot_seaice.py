...
def test_check_list_regions_type(regions_to_plot, expect_exception, error_message, expected_output):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, None, None, None),
        ("string", ValueError, "Invalid data type", None),
        ([1, 2, 3], ValueError, "Invalid data type", None),
    ]
)
def test_check_as_datasets_list(datain, expect_exception, error_message, expected_output):
...
def test_repack_datasetlists(input_datasets, regions_to_plot, expected_output):
...
'''
def test_plot_seaice_timeseries():
